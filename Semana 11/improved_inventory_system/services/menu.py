...
class Menu:
    ...
    def show_menu(self):
        while True:
            ...
            print("🗑️ 2. Eliminar producto")
            print("✏️ 3. Actualizar producto")
            ...
            if option == "1":
                ...
                print("\n✖️ Opción no válida. Intente nuevamente.") # Condicional para manejar opciones no válidas ingresadas por el usuario
    ...
    def add_product(self):
        ...
        product_id = input("🆔 Ingrese el ID del producto: ").strip().upper() # Solicita el ID del producto al usuario y valida que no esté vacío
        validate_id, message_id = Validator.validate_id(product_id)
        if not validate_id:
            print(f"✖️ Error: {message_id}")
            return

        # Valida si el ID ya existe
        if self.inventory.find_by_id(product_id):
            print(f"\n⚠️ El producto con ID '{product_id}' ya existe. No puede registrarse nuevamente 👀")
            return
        ...
        if not validate_quantity:
            print(f"✖️ Error: {message_quantity}")
            return
    ...
    def remove_product(self):
        ...
        if product:
            ...
            print(f"\n✖️ Producto con ID '{product_id}' no encontrado. Verifique que el ID sea correcto 👀")
    ...
    def update_product(self):
        ...
        print(f"🛍️ Nombre: {product.get_name()}")
        print("\n🛠️ Actualice únicamente los campos necesarios (deje vacío para omitir).")
        ...
        quantity = input("\n🔢 Nueva cantidad: ").strip()
        price = input("💲Nuevo precio unitario: ").strip()
    ...
    def search_product(self):
        ...
        # Condicional que verifica la opción de búsqueda seleccionada por el usuario, solicitando el nombre del producto a buscar y se valida antes de realizar la búsqueda por nombre en el inventario, permitiendo coincidencias parciales y no distinguiendo entre mayúsculas y minúsculas
            ...
            name = input("🛍️ Ingrese el nombre del producto a buscar: ").strip()
            validate_name, message_name = Validator.validate_name(name)
            if not validate_name:
                print(f"\n❌ Error: {message_name}")
                return
    ...
    def list_products(self):
        ...
        if products:
            ...
            print("\n📦 El inventario actualmente está vacío.")
